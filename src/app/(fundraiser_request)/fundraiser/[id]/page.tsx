/* eslint-disable react/no-unescaped-entities */
import { Box, Container, Flex, Title } from "@mantine/core";
import React from "react";
import { BackBtn } from "../../components/BackBtn";
import { DonationCard } from "../DonationCard";
import { Slider } from "../Slider";
import { RequestInfo } from "../RequestInfo/RequestInfo";
import { CollectedGraphic } from "../../components/CollectedGraphic/CollectedGraphic";
import { DonateMethInfo } from "../../components/DonateMethInfo";
import { Table, Warning } from "@/shared/ui";

const img =
  "data:image/jpeg;base64,";

interface IFundraiserPage {
  params: {
    id: string;
  };
}

const FundraiserPage = ({ params }: IFundraiserPage) => {

  
  return (
    <Container size="xl">
      <Box h={22} />
      <BackBtn />
      <Box h={22} />
      <Flex
        gap={{ base: 20, md: 30, lg: 34 }}
        direction={{ base: "column-reverse", lg: "row" }}
      >
        <DonationCard />
        <Box>
          <Flex
            gap={{ base: 20, md: 30, lg: 34 }}
            direction={{ base: "column-reverse", md: "row" }}
            mb={{ base: 40, md: 100 }}
          >
            <Slider images={[img, img]} />
            <RequestInfo />
          </Flex>
          <CollectedGraphic
            need={"10 000 000"}
            collected={"10 000 000"}
            currency={"UAH"}
            remains={"10 000 000"}
          />
          <Box h={{ base: 40, md: 100 }} />
          <Flex gap={46} mb={{ base: 40, lg: 100 }} direction={{base: 'column', md: 'row'}} >
            <Title order={2} maw={222}>
              Why is it important?
            </Title>
            <Box flex="1 1 auto">
              <Box>
                The effectiveness of BpA calculation directly depends on the
                control point. Mobile command posts from the "League of aerial
                reconnaissance 3.0" will be equipped on the basis of new and
                four-wheel-drive vehicles of the bus type, which will be able to
                deliver crews to task areas under any conditions and, if
                necessary, quickly change the place of deployment. The cars that
                we will purchase will have comfortable workplaces, the necessary
                power supply, communication equipment, climate control necessary
                for the work of external pilots and "iron", as well as special
                places for the transportation of BPAKs and all the necessary
                equipment for their management. In addition to the "Come Back
                Alive" website, 1+1 media will also accumulate funds during the
                broadcasts of all football matches of the Ukrainian Premier
                League (UPL), which will last from February 23 to May 25, 2024.
                The matches will be broadcast by UPL TV, 2+2, UNIAN, as well as
                the 1+1 media digital sports project — FootballHub. As a
                reminder, within the framework of the first two stages of the
                "League of aerial reconnaissance" project, together with our
                partners, we strengthened 9 divisions of aerial reconnaissance
                for UAH 32,086,716.6. At the end of 2023, we handed over to the
                military 5 mobile control points for unmanned systems based on a
                four-wheel drive minibus.
              </Box>
              <Box h={{ base: 40, md: 100 }} />
              <DonateMethInfo
                category={"Wallet"}
                walletType={"Payoneer"}
                cardNum={"0000 0000 0000 0000"}
                title={"Requisites:"}
                names="Full User`s Name"
                copy
              />
            </Box>
          </Flex>
        </Box>
      </Flex>
      <Box h={{ base: 40, md: 100 }} />
      <Title order={1} mb={24}>
        All receipts
      </Title>
      <Warning
        title={"THE SPREADSHEET DATA SERVES AN INFORMATIVE PURPOSE"}
        subtitle={
          "We'd like to draw your attention to the fact that access to public information, as well as information regarding the intended use of charitable funds, is governed by the current Ukrainian legislation."
        }
      />
      <Box h={24} />

      <Table
        elements={[
          [
            "Date, time",
            "Source",
            "Name of fundraising",
            "Amount, uAH",
            "Currency",
            "Comment",
          ],
          ["-", "-", "-", "-", "-", "-"],
        ]}
      />
      <Box h={{ base: 40, md: 100 }} />
    </Container>
  );
};

export default FundraiserPage;
